...
)
{
	...
	const bool bDynamic = (usage == D3D11_USAGE_DYNAMIC);

    // Create the color texture
    int createFlags = createUAV 
        ? (bDynamic? 0 : D3D11_BIND_RENDER_TARGET) | D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_UNORDERED_ACCESS
		: (bDynamic ? 0 : D3D11_BIND_RENDER_TARGET) | D3D11_BIND_SHADER_RESOURCE;
    ...
    mColorDesc = CD3D11_TEXTURE2D_DESC(
        ...
        depth, // Array Size
        1, // MIP Levels
        createFlags, // bindFlags
		usage,// D3D11_USAGE
		bDynamic? D3D11_CPU_ACCESS_WRITE : 0, // cpuaccessFlags
        mMultiSampleCount, 
		0
    );
	...
	subresource.pSysMem = (const void*) pData;
    ...
    srDesc.Texture2D.MipLevels = 1;
	if (srDesc.Format == DXGI_FORMAT_YUY2)
...
//-----------------------------------------------------------------------------
D3D11_MAPPED_SUBRESOURCE CPUTRenderTargetColor::MapRenderTarget( 
	...
	)
{
	...
	const bool bDynamic = (mColorDesc.Usage == D3D11_USAGE_DYNAMIC);

    if( !mpColorTextureDXStaging && !bDynamic) // if dynamic or staging, use the resource created in CreateRenderTarget (don't create a staging buffer again)
    {
        ...
        case CPUT_MAP_NO_OVERWRITE:
			...
			desc.BindFlags = 0;
			// Assert below isn't correct: we can create a staging texture for use if the original resource is bound as render target (with default usage)
			//ASSERT(0, "DirectX does not allow MAP_WRITE_DISCARD of NO_OVERWRITE for render targets.");
            break;
        };
	...
	// if resource is marked with dynamic usage, use the original resource and not the staging copy
	UINT subresource = D3D11CalcSubresource(0, arraySlice, mColorDesc.MipLevels);

	hr = pContext->Map(
		bDynamic? mpColorTextureDX : mpColorTextureDXStaging, 
		subresource,
		(D3D11_MAP)type, 
		wait ? 0 : D3D11_MAP_FLAG_DO_NOT_WAIT, 
		&info);

	ASSERT(SUCCEEDED(hr), "Failed to map render target");
...
void CPUTRenderTargetColor::UnmapRenderTarget( CPUTRenderParameters &params )
{
	...
	// when using dynamic, we avoid the staging buffer and having to copy (using the gpu) from it to the render target
	if (bDynamic) {
		pContext->Unmap(mpColorTextureDX, 0);
		return;
	}
