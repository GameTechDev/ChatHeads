...
// -------------------------------------
    ...
    Texture2D texture_EM : register( t4 );
#else
...
sampler2D SAMPLER4 = sampler_state{ texture = (texture_EM); MinFilter = Linear; MagFilter = Linear; MipFilter = Linear; };
bool texture_EMsRGB = true;
...
#else
SRGB2Linear(tex2D( SAMPLER1, ((input.UV0)) ), texture_DMsRGB)
#endif
;
}
...
#else
SRGB2Linear(tex2D( SAMPLER2, ((input.UV0)) ), texture_SMsRGB)
#endif
;
}
...
float4 SPECULARITY( PS_INPUT input )
{
    return ((( (
#ifdef _CPUT
texture_ST.Sample( SAMPLER0, ((input.UV0)) )
#else
SRGB2Linear(tex2D( SAMPLER0, ((input.UV0)) ), texture_STsRGB)
#endif
...
#else
SRGB2Linear(tex2D( SAMPLER3, ((input.UV0)) ), texture_NMsRGB)
#endif
 * 2.0 - 1.0;
}
...
float4 EMISSIVETMP( PS_INPUT input )
{
    return 
#ifdef _CPUT
texture_EM.Sample( SAMPLER0, ((input.UV0)) )
#else
SRGB2Linear(tex2D( SAMPLER4, ((input.UV0)) ), texture_EMsRGB)
#endif
;
}
...
{
    return (EMISSIVETMP(input)) *(0.000000);
}

// -------------------------------------
float4 SPECULAR( PS_INPUT input )
{
    return (SPECULARTMP(input)) *(5.000000);
}
...
{
    return ( (DIFFUSE(input) ).aaaa );
}

// -------------------------------------
float4 CLIPTHRESHOLD( PS_INPUT input )
{
    return float4(0.500000, 0.500000, 0.500000, 0.500000);
}
