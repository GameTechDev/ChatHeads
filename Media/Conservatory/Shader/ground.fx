...
sampler2D SAMPLER1 = sampler_state{ texture = (texture_NM); MinFilter = Linear; MagFilter = Linear; MipFilter = Linear; };
...
float4 DIFFUSE( PS_INPUT input )
{
    return 
#ifdef _CPUT
texture_DM.Sample( SAMPLER0, (((input.UV0)) *(1.000000)) )
#else
SRGB2Linear(tex2D( SAMPLER0, (((input.UV0)) *(1.000000)) ), texture_DMsRGB)
#endif
;
}
...
float4 NORMALS( PS_INPUT input )
{
    return 
#ifdef _CPUT
texture_NM.Sample( SAMPLER0, (((input.UV0)) *(10.000000)) )
#else
SRGB2Linear(tex2D( SAMPLER1, (((input.UV0)) *(10.000000)) ), texture_NMsRGB)
#endif
 * 2.0 - 1.0;
}
...
{
    return float3(0.100000, 0.100000, 0.100000).xyzz;
}
