...
/*
Material defines vec4 AMBIENT(), vec4 DIFFUSE(), vec4 SPECULAR() colors
and float SPECULARITY() 
*/
<Material Definition>
...
void main( )
{
...
#ifdef USE_NORMALMAP
    vec3 normalTex = NORMAL().bgr;
    mat3 tangentToWorld = mat3(outTangent, outBinormal, outNormal);
    vec3 normal = normalize( tangentToWorld * normalTex );
#else
	...
	fragColor = result;
	/* Test normals 
    fragColor =  vec4(	clamp(dot(normal, lightDirection), 0.0, 1.0),
						clamp(dot(normal, -lightDirection), 0.0, 1.0),
						0.0,
						1.0);
						*/
